...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, mask_method="patches"):
        super().__init__()
    ...
    def patchify(self, imgs, channels=3):
        """
        imgs: (N, 3, H, W)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        if self.mask_method in ["segments", "four_channels", "preencoder"]:
            mask = self.patchify(mask, channels=1)
            mask = mask.repeat((1,1,3))
        elif self.mask_method == "patches":
